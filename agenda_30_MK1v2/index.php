<html>
    ...
    <body>
        ...
        <div class="doce_de_leite_mumu_o_da_vaquinha" style="background-color:#0D0F12; width:450px; margin:0 auto; border-radius:15px; box-shadow: 0 0 0 0.3px #818898; padding:20px;">
            ...
            <form action="salvar.php" id="formes" method="post" style="color:#f8f8ff;">
                <p style="color:#818898; font-weight: 600; font-size:18px;"> NOME: </p> <input type="text" name="nome" style="width:100%; box-sizing:border-box; background-color:#2a2a2a; color: #FFFFFF; padding:10px; box-shadow: 0 0 0 0.5px #818898; border: none; border-radius:4px;" required> <br><br>
                <p style="color:#818898; font-weight: 600; font-size:18px;"> ENDEREÇO: </p> <input type="text" name="endereco" style="width:100%; box-sizing:border-box; background-color:#2a2a2a; color: #FFFFFF; padding:10px; box-shadow: 0 0 0 0.5px #818898; border: none; border-radius:4px;" required> <br><br>
                <p style="color:#818898; font-weight: 600; font-size:18px;"> TELEFONE: </p> <input type="text" name="fone" id="AAA" oninput="maskAndValidatePhone(input)" required style="width:100%; box-sizing:border-box; background-color:#2a2a2a; color: #FFFFFF; padding:10px; box-shadow: 0 0 0 0.5px #818898; border: none; border-radius:4px;"> <br><br>
                <button type="submit" style="background-color:#2fdda3; color:#000000; border: none; padding:12px; width:100%; border-radius:4px; cursor:pointer;"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-plus" viewBox="0 0 16 16"> <path d="M6 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6m2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0m4 8c0 1-1 1-1 1H1s-1 0-1-1 1-4 6-4 6 3 6 4m-1-.004c-.001-.246-.154-.986-.832-1.664C9.516 10.68 8.289 10 6 10s-3.516.68-4.168 1.332c-.678.678-.83 1.418-.832 1.664z"/> <path fill-rule="evenodd" d="M13.5 5a.5.5 0 0 1 .5.5V7h1.5a.5.5 0 0 1 0 1H14v1.5a.5.5 0 0 1-1 0V8h-1.5a.5.5 0 0 1 0-1H13V5.5a.5.5 0 0 1 .5-.5"/> </svg> CADASTRAR </button>
            </form>

        </div>

        <div class="acucar_mascavo" style="width:450px; margin:0 auto; margin-top:20px; margin-bottom:20px;">
                                    ...
                                    while ($linha = mysqli_fetch_assoc($resultado)){
                                        echo " <tr style='border-bottom: 1px solid #24262b;'>
                                            ...
                                            <td style='padding: 8px; white-space: normal;'>
                                                <a href='editar.php?id=" . $linha['id'] . "' style='color:#24262b; background-color:#00ffcc; padding: 2px 7px; border-radius: 5px; text-decoration: none; font-weight: 600; display:inline-block; margin-right:5px; width: 100%; text-align: center;'> EDITAR </a> 
                                                <a href='excluir.php?id=" . $linha['id'] . "' onclick='return confirm(\"Você quer realmente excluir o contato? \");' style='color:#f8f8ff; background-color: #b22222; padding: 2px 7px; margin-top:5px; border-radius: 5px; text-decoration: none; font-weight: 600; display:inline-block; width: 100%; text-align: center;'> EXCLUIR </a>
                                            </td>
                                        </tr> ";
                                    }
                                    echo "</table>";
        ...
        <script> 
            function maskAndValidatePhone(input) {
                // Remove caracteres que não são números
                let numericValue = input.value.replace(/\D/g, '');
                
                // Adiciona formatação de máscara opcional (ex: 55 11 91234-5678)
                if (numericValue.length > 2) {
                    numericValue = numericValue.replace(/^(\d{2})(\d+)/, '$1 $2');
                }
                if (numericValue.length > 10) {
                    numericValue = numericValue.replace(/(\d{5})(\d{4})$/, '$1-$2');
            ...
            // Seleciona o input
            const phoneInput = document.getElementById('AAA');

            // Evento de input para aplicar a máscara em tempo real
            phoneInput.addEventListener('input', () => maskAndValidatePhone(phoneInput));

            // Validação no formulário antes de enviar
            document.getElementById('formes').addEventListener('submit', (event) => {
                event.preventDefault(); // Previne envio se inválido

                const numericValue = phoneInput.value.replace(/\D/g, ''); // limpa máscara
                if (numericValue.length < 14 || numericValue.length > 15) {
                    alert('O telefone deve ter entre 14 e 15 caracteres (somente números).');
                    return;
                }

                // Se estiver válido, envia (ou prossiga com processamento)
                alert('Telefone válido: ' + numericValue);
            });
        </script>
    </body>
</html>